...
void set_cpu_affinity(pthread_t thread, int core_id){
    ...
    if (s != 0) {
        fprintf(stderr, "Error to define affnitty to core %d\n", core_id);
    }
}
...
void signal_rcv_config(){
	...
	siginfo_t data_rcv;
	for(int i=1; i < NF_CONTAINERS && (sigwaitinfo(&set2, &data_rcv) > 0) ; i++){
...
static int gateway(void)
{
    ...
    uint32_t index = (HASH_SIZE - 1) & pid_gateway;
...
int main(int argc, char **argv)
{
    ...
    //preheat_mempool(sigbpf_ptr, N_ELEMENTS * sizeof(struct http_transaction));
