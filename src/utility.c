...
{
    sigbpf_cfg->inter_node_rt[fn_id] = node_idx;
}

uint8_t get_node(uint8_t fn_id)
{
    uint8_t peer_node_idx = sigbpf_cfg->inter_node_rt[fn_id];

    log_debug("Destination function is %u on node %u (%s:%u).", fn_id, peer_node_idx,
              sigbpf_cfg->nodes[peer_node_idx].ip_address, INTERNAL_SERVER_PORT);
...
{
    sigbpf_cfg->inter_node_rt[fn_id] = 0;
}
...
void print_rt_table()
{
    ...
    for (int i = 1; i <= sigbpf_cfg->n_nfs; i++)
    {
        printf("\tFn: %d, Node: %d\n", i, sigbpf_cfg->inter_node_rt[i]);
    }
}
