...
if [ -z "$SIGSHARED_PATH" ] ; then
  echo "Usage: $ <SIGSHARED_PATH>"
  exit 1
fi
...
echo "Testing S-SIGSHARED with parking detection dataset..."
...
pidstat 1 700 -G ^gateway_$ > sigshared_gw.parking.cpu & pidstat 1 700 -G ^nf_$ > sigshared_fn.parking.cpu
