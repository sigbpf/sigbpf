...
{
    if (sigbpf_cfg->n_tenants == 0)
        return 0;

    int result = sigbpf_cfg->tenants[0].weight;

    for (int i = 1; i < sigbpf_cfg->n_tenants; i++)
    {
        result = gcd(result, sigbpf_cfg->tenants[i].weight);
    }
...
{
    int max_val = sigbpf_cfg->tenants[0].weight;

    for (int i = 1; i < sigbpf_cfg->n_tenants; i++)
    {
        if (sigbpf_cfg->tenants[i].weight > max_val)
        {
            max_val = sigbpf_cfg->tenants[i].weight;
        }
    }
...
{
    int num_tenants = sigbpf_cfg->n_tenants;
    ...
    for (int i = 0; i < num_tenants; i++)
    {
        ...
        tenant_pipes[i].weight = sigbpf_cfg->tenants[i].weight;
...
int add_weighted_pipes_to_epoll(int epoll_fd, struct epoll_event *ev)
{
    ...
    for (int i = 0; i < sigbpf_cfg->n_tenants; i++)
    {
