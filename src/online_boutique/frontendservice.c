...
static int nf(uint8_t nf_id){
    ...
    matriz[nf_id][1] = pid;
    if(unlikely( sigbpf_update_map("pid_map", fn_id, pid, &mapa_fd) < 0 ) ){
    ...
    if( bpf_map_lookup_elem(fd_map_signal, &key_gateway, &pid_gateway ) < 0 ){
...
int main(int argc, char **argv){
    ...
    log_info("Pinning process in CPU 2...\n");
    sprintf(settar_cpuf, "taskset -cp 2 %d", pid);
