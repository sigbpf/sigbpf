/*
 * This file has been simplified and modified from the original project SPRIGHT for the purposes of test and experiment the latency impact of real-time signals in microsservices.
 *
 */



/*
# Copyright 2025 University of California, Riverside
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#      http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
# SPDX-License-Identifier: Apache-2.0
*/

#define _GNU_SOURCE
#include <errno.h>
#include <math.h>
#include <pthread.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/epoll.h>
#include <time.h>
#include <unistd.h>
#include <sched.h>

#include "./sigbpf.h"
#include "./include/io.h"


sigset_t set, set2;
int matriz[20][2] = {0};
int pid, mapa_fd, fd_pid_map;

int hash_table[HASH_SIZE];

#include "./include/http.h"
#include "./include/io.h"
#include "./log/log.h"
#include "./include/spright.h"

#include "sigbpf.h"

static int pipefd_rx[UINT8_MAX][2];
static int pipefd_tx[UINT8_MAX][2];

static int autoscale_memory(uint8_t mb)
{
    char *buffer = NULL;

    if (unlikely(mb == 0))
    {
        return 0;
    }

    buffer = malloc(1000000 * mb * sizeof(char));
    if (unlikely(buffer == NULL))
    {
        log_error("malloc() error: %s", strerror(errno));
        return -1;
    }

    buffer[0] = 'a';
    buffer[1000000 * mb - 1] = 'a';

    free(buffer);

    return 0;
}

static int autoscale_sleep(uint32_t ns)
{
    struct timespec interval;
    int ret;

    interval.tv_sec = ns / 1000000000;
    interval.tv_nsec = ns % 1000000000;

    ret = nanosleep(&interval, NULL);
    if (unlikely(ret == -1))
    {
        //log_error("nanosleep() error: %s", rte_strerror(errno));
        return -1;
    }

    return 0;
}

static int autoscale_compute(uint32_t n)
{
    uint32_t i;

    for (i = 2; i < sqrt(n); i++)
    {
        if (n % i == 0)
        {
            break;
        }
    }

    return 0;
}

static void *nf_worker(void *arg)
{
    struct http_transaction *txn = NULL;
    ssize_t bytes_written;
    ssize_t bytes_read;
    uint8_t index;
    int ret;

    /* TODO: Careful with this pointer as it may point to a stack */
    index = (uint64_t)arg;

    while (1)
    {
        bytes_read = read(pipefd_rx[index][0], &txn, sizeof(struct http_transaction *));
        if (unlikely(bytes_read == -1))
        {
            log_error("read() error: %s", strerror(errno));
            return NULL;
        }

        //log_debug("Fn#%d is processing request.", fn_id);

        ret = autoscale_memory(sigbpf_cfg->nf[fn_id - 1].param.memory_mb);
        if (unlikely(ret == -1))
        {
            log_error("autoscale_memory() error");
            return NULL;
        }

        ret = autoscale_sleep(sigbpf_cfg->nf[fn_id - 1].param.sleep_ns);
        if (unlikely(ret == -1))
        {
            log_error("autoscale_sleep() error");
            return NULL;
        }

        ret = autoscale_compute(sigbpf_cfg->nf[fn_id - 1].param.compute);
        if (unlikely(ret == -1))
        {
            log_error("autoscale_compute() error");
            return NULL;
        }

        bytes_written = write(pipefd_tx[index][1], &txn, sizeof(struct http_transaction *));
        if (unlikely(bytes_written == -1))
        {
            log_error("write() error: %s", strerror(errno));
            return NULL;
        }
    }

    return NULL;
}



static void *nf_rx(void *arg){
    struct http_transaction *txn = NULL;
    ssize_t bytes_written;
    uint8_t i;


    for (i = 0;; i = (i + 1) % sigbpf_cfg->nf[fn_id - 1].n_threads){
	wait_for_ownership( (void **)&txn, sigbpf_ptr, &set,  hash_t);

        if (!txn){
            log_error("io_rx() error: sigbpf_mempool_access() returned NULL");
            return NULL;
        }

        bytes_written = write(pipefd_rx[i][1], &txn, sizeof(struct http_transaction *));
        if (unlikely(bytes_written == -1)){
            log_error("write() error: %s", strerror(errno));
            return NULL;
        }
    }

    return NULL;
}

static void *nf_tx(void *arg){
    struct epoll_event event[UINT8_MAX]; /* TODO: Use Macro */
    struct http_transaction *txn = NULL;
    ssize_t bytes_read;
    uint8_t i;
    int n_fds;
    int epfd;
    int ret;
    int pid = getpid();

    epfd = epoll_create1(0);
    if (unlikely(epfd == -1))
    {
        log_error("epoll_create1() error: %s", strerror(errno));
        return NULL;
    }

    for (i = 0; i < sigbpf_cfg->nf[fn_id - 1].n_threads; i++)
    {
        ret = set_nonblocking(pipefd_tx[i][0]);
        if (unlikely(ret == -1))
        {
            return NULL;
        }

        event[0].events = EPOLLIN;
        event[0].data.fd = pipefd_tx[i][0];

        ret = epoll_ctl(epfd, EPOLL_CTL_ADD, pipefd_tx[i][0], &event[0]);
        if (unlikely(ret == -1))
        {
            log_error("epoll_ctl() error: %s", strerror(errno));
            return NULL;
        }
    }

    while (1)
    {
        n_fds = epoll_wait(epfd, event, sigbpf_cfg->nf[fn_id - 1].n_threads, -1);
        if (unlikely(n_fds == -1))
        {
            log_error("epoll_wait() error: %s", strerror(errno));
            return NULL;
        }

        for (i = 0; i < n_fds; i++)
        {
            bytes_read = read(event[i].data.fd, &txn, sizeof(struct http_transaction *));
            if (unlikely(bytes_read == -1))
            {
                log_error("read() error: %s", strerror(errno));
                return NULL;
            }

            txn->hop_count++;

            if (likely(txn->hop_count < sigbpf_cfg->route[txn->route_id].length))
            {
                txn->next_fn = sigbpf_cfg->route[txn->route_id].hop[txn->hop_count];
            }
            else
            {
                txn->next_fn = 0;
            }

            log_debug("Route id: %u, Hop Count %u, Next Hop: %u, Next Fn: %u, Caller Fn: %s (#%u), RPC Handler: %s()",
                      txn->route_id, txn->hop_count, sigbpf_cfg->route[txn->route_id].hop[txn->hop_count], txn->next_fn,
                      txn->caller_nf, txn->caller_fn, txn->rpc_handler);

	    ret = transfer_ownership(txn->addr, txn->next_fn, &map_fd, pid, matriz[txn->next_fn][1], hash_t);
            if (unlikely(ret == -1))
            {
                log_error("io_tx() error");
                return NULL;
            }
        }
    }

    return NULL;
}


void set_cpu_affinity(pthread_t thread, int core_id){

    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(core_id, &cpuset);

    int s = pthread_setaffinity_np(thread, sizeof(cpu_set_t), &cpuset);
    if (s != 0) {
        fprintf(stderr, "Erro ao definir afinidade no Core %d\n", core_id);
    }
}

/* TODO: Cleanup on errors */
static int nf(uint8_t nf_id, uint8_t cpu_nf_id)
{
    pthread_t thread_worker[UINT8_MAX];
    pthread_t thread_rx;
    pthread_t thread_tx;
    uint8_t i;
    int ret;
    uint32_t index;

    fn_id = nf_id;

    pid = getpid();
    matriz[nf_id][1] = pid;
    if( sigbpf_update_map("pid_map", fn_id, pid, &mapa_fd) < 0  ){
        printf("Erro ao atualizar mapa\n");
        return 0;
    }

    index = (HASH_SIZE - 1) & pid;
    hash_t[index].pid = pid; 
    hash_t[index].index = index; 


    char temp[256];
    char *dir_temp = getenv("SIGBPF");
    sprintf(temp, "%s/dados/pid_map", dir_temp);
    fd_pid_map = bpf_obj_get(temp);

    int pid_gw, key_front = 0;
    if( bpf_map_lookup_elem(fd_pid_map, &key_front, &pid_gw ) < 0 ){
	    perror("Error to lookup the gateway's PID");
	    exit(1);
    }

    index = (HASH_SIZE - 1) & pid_gw;
    hash_table[index] = pid_gw; 
    hash_t[index].pid = pid_gw; 
    hash_t[index].index = index; 

    matriz[0][1] = pid_gw;
    sigval_t data_send;
    data_send.sival_ptr = (void *)(uintptr_t)nf_id;

    // Send SIGRTMIN+2 to gateway 
    if(sigqueue(pid_gw, SIGRTMIN+2, data_send) < 0){
        perror("Error to send signal to gateway");
    }

    siginfo_t data_rcv;
    if( sigwaitinfo(&set, &data_rcv) > 0 ){
	log_info("Sinal recebido do gateway!");
    	lookup_map_nf(&fd_pid_map, matriz);
    }


    for (i = 0; i < sigbpf_cfg->nf[fn_id - 1].n_threads; i++)
    {
        ret = pipe(pipefd_rx[i]);
        if (unlikely(ret == -1))
        {
            log_error("pipe() error: %s", strerror(errno));
            return -1;
        }

        ret = pipe(pipefd_tx[i]);
        if (unlikely(ret == -1))
        {
            log_error("pipe() error: %s", strerror(errno));
            return -1;
        }
    }

    ret = pthread_create(&thread_rx, NULL, &nf_rx, NULL);
    if (unlikely(ret != 0))
    {
        log_error("pthread_create() error: %s", strerror(ret));
        return -1;
    }

    ret = pthread_create(&thread_tx, NULL, &nf_tx, NULL);
    if (unlikely(ret != 0))
    {
        log_error("pthread_create() error: %s", strerror(ret));
        return -1;
    }


    struct sched_param sp = { .sched_priority = 50 };
    pthread_setschedparam(thread_rx, SCHED_FIFO, &sp);
    pthread_setschedparam(thread_tx, SCHED_FIFO, &sp);


    log_info("Creating %d workers...", sigbpf_cfg->nf[fn_id -1].n_threads );
    for (i = 0; i < sigbpf_cfg->nf[fn_id - 1].n_threads; i++)
    {
        ret = pthread_create(&thread_worker[i], NULL, &nf_worker, (void *)(uint64_t)i);
        if (unlikely(ret != 0))
        {
            log_error("pthread_create() error: %s", strerror(ret));
            return -1;
        }
	set_cpu_affinity(thread_worker[i], cpu_nf_id);
	struct sched_param sp = { .sched_priority = 50 };
	pthread_setschedparam(thread_worker[i], SCHED_FIFO, &sp);

    }

    for (i = 0; i < sigbpf_cfg->nf[fn_id - 1].n_threads; i++)
    {
        ret = pthread_join(thread_worker[i], NULL);
        if (unlikely(ret != 0))
        {
            log_error("pthread_join() error: %s", strerror(ret));
            return -1;
        }
    }

    ret = pthread_join(thread_rx, NULL);
    if (unlikely(ret != 0))
    {
        log_error("pthread_join() error: %s", strerror(ret));
        return -1;
    }

    ret = pthread_join(thread_tx, NULL);
    if (unlikely(ret != 0))
    {
        log_error("pthread_join() error: %s", strerror(ret));
        return -1;
    }

    for (i = 0; i < sigbpf_cfg->nf[fn_id - 1].n_threads; i++)
    {
        ret = close(pipefd_rx[i][0]);
        if (unlikely(ret == -1))
        {
            log_error("close() error: %s", strerror(errno));
            return -1;
        }

        ret = close(pipefd_rx[i][1]);
        if (unlikely(ret == -1))
        {
            log_error("close() error: %s", strerror(errno));
            return -1;
        }

        ret = close(pipefd_tx[i][0]);
        if (unlikely(ret == -1))
        {
            log_error("close() error: %s", strerror(errno));
            return -1;
        }

        ret = close(pipefd_tx[i][1]);
        if (unlikely(ret == -1))
        {
            log_error("close() error: %s", strerror(errno));
            return -1;
        }
    }

    return 0;
}

int main(int argc, char **argv)
{
    log_set_level_from_env();

    uint8_t nf_id;
    int ret;

    char settar_cpuf[200];

    uint8_t cpu_nf_id = strtol(argv[argc-1], NULL, 10) ;
    cpu_nf_id = cpu_nf_id >= 7 ? (cpu_nf_id + 13) : cpu_nf_id + 1;

    log_info("Atribuindo processo para a CPU %d...\n", cpu_nf_id);
    sprintf(settar_cpuf, "taskset -cp %d %d", cpu_nf_id, getpid());
    int ret_sys = system(settar_cpuf);
    if( ret_sys == -1 || ret_sys == 127 ){
            log_error("Error to pinn CPU");
            exit(1);
    }

    
    for(int i=0; i<NF_CONTAINERS; i++){
	    matriz[i][1] = 0;
    }

    sigbpf_ptr = sigbpf_ptr_mem();
    if(sigbpf_ptr == NULL){
	    log_error("ERRO NO sigbpf_ptr");
	    return 1;
    }

    sigbpf_cfg = sigbpf_cfg_ptr();
    if(sigbpf_cfg == NULL){
        log_error("ERRO NO sigbpf_cfg");
        return 1;
    }


    sigemptyset(&set);       
    //sigaddset(&set, SIGRTMIN+1); 
    sigaddset(&set, SIGBPF); 
    sigaddset(&set, SIGRTMIN+2); 
    sigprocmask(SIG_BLOCK, &set, NULL);

    errno = 0;
    nf_id = strtol(argv[argc-1], NULL, 10);
    printf("NF_ID: %d\n", nf_id);
    if (unlikely(errno != 0 || nf_id < 1))
    {
        log_error("Invalid value for Network Function ID");
        goto error_1;
    }

    for (int i=0; i< 20; i++){
	    matriz[i][1] = 0;
    }

    ret = nf(nf_id, cpu_nf_id);
    if (unlikely(ret == -1))
    {
        log_error("nf() error");
        goto error_1;
    }

    return 0;

error_1:
    printf("Erro ao inicializar nf()\n");
//error_0:
//    return 1;
}
